...
def show_autoencoder_results(model, test_loader, num_images=100, images_per_row=10, save_dir='plots'):
    ...
    average_class_errors = {cls: (sum(errors) / len(errors)) if len(errors) > 0 else 0 for cls, errors in
                            class_errors.items()}
    ...
    plt.savefig(os.path.join(save_dir, 'autoencoder_class_errors.png'))
    plt.show()
    ...
    # Save the results
    plt.savefig(os.path.join(save_dir, 'autoencoder_results.png'))
    plt.show()